...
mod tests {
    ...
    // Reproducible native generator and fixture descriptions live beside these
    // JPEGs in testdata/generate_twitter_capacity_fixtures.cpp and README.md.
    const LOW_AMPLITUDE_COVER: &[u8] = include_bytes!("testdata/twitter_low_amplitude.jpg");
    const CARRIER_KEY: u64 = 42;

    #[test]
    fn full_capacity_survives_nonzero_coefficient_loss() {
        let cover = prepare_cover(LOW_AMPLITUDE_COVER).unwrap();
        assert_eq!((cover.width, cover.height), (400, 400));
        assert_eq!(cover.payload_capacity, 7851);

        let kdf = vec![0x5a; KDF_METADATA_REGION_BYTES];
        let encrypted = vec![0x42; cover.payload_capacity - TWITTER_ENVELOPE_HEADER_BYTES];
        let payload = make_envelope(&kdf, &encrypted, true).unwrap();
        assert_eq!(payload.len(), cover.payload_capacity);
        let embedded = cover.embed(CARRIER_KEY, &payload).unwrap();

        // Embedding changes some +1 AC coefficients to zero. Re-budgeting the
        // output as a new cover must demonstrate the previously fatal decrease.
        let after = prepare_cover(&embedded).unwrap();
        assert!(after.payload_capacity < payload.len());
        let recovered = extract_envelope(&embedded, CARRIER_KEY)
            .unwrap()
            .expect("full-capacity carrier was not recognized");
        assert_eq!(recovered.kdf_metadata, kdf);
        assert_eq!(recovered.encrypted_data, encrypted);
        assert!(recovered.is_compressed);
        assert!(extract_envelope(&embedded, CARRIER_KEY + 1)
            .unwrap()
            .is_none());
    }

    #[test]
    fn oversized_keyed_headers_fail_before_payload_allocation() {
        // Both JPEGs contain a valid header CRC and key. Their lengths are one
        // byte beyond the invariant symbol capacity and u32::MAX, respectively.
        for malformed in [
            include_bytes!("testdata/twitter_oversized_header.jpg").as_slice(),
            include_bytes!("testdata/twitter_maximum_header.jpg").as_slice(),
        ] {
            let error = extract_envelope(malformed, CARRIER_KEY)
                .err()
                .expect("oversized carrier header was accepted");
            assert!(
                error.contains("larger than its coefficient capacity"),
