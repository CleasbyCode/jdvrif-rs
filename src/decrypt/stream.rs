use crate::common::{
    ...
    open_binary_output_for_staging_or_throw,
};
use crate::constants::{
...
impl<W: Write> StreamingInflater<W> {
    fn new(output: W, max_output: usize) -> Self {
        Self {
            ...
            output,
            output_buf: Zeroizing::new(vec![0u8; 2 * 1024 * 1024]),
            output_size: 0,
            max_output,
            stream_ended: false,
        }
    ...
    fn finish(self) -> Result<(W, usize), String> {
        if !self.stream_ended {
            return Err("zlib inflate error: compressed stream is truncated".to_string());
        }
        // The writer is returned unflushed on purpose: the caller owns closing
        // it, and only a checked close can surface a deferred write error.
...
pub(super) fn decrypt_ciphertext_to_stage_file(
    ...
    let associated_data = if metadata_version.authenticates_stream_mode() {
    ...
    if is_data_compressed {
        // stream_stage_path is a StagingFile (link-free inode reached through
        // /proc/self/fd): the inode already exists, so this truncates rather
        // than exclusively creating a new named file.
        let output_file = open_binary_output_for_staging_or_throw(stream_stage_path)?;
        let mut inflater =
        ...
        let (writer, output_size) = inflater.finish()?;
        // Durable, checked close before the size cross-check and before the file
        // is committed under its recovered name and reported as extracted.
        finish_buffered_file(writer, WRITE_COMPLETE_ERROR, true)?;
    ...
    // Durable, checked close before the file is committed under its recovered
    // name and reported as extracted.
    finish_buffered_file(output, WRITE_COMPLETE_ERROR, true)?;
...
mod tests {
    ...
    static WATCHED_BUFFER: AtomicPtr<u8> = AtomicPtr::new(std::ptr::null_mut());
    static BUFFER_WAS_WIPED: AtomicBool = AtomicBool::new(false);

    // Inspect only the explicitly registered, fully initialized inflater buffer,
    // immediately before releasing it. The allocator must not allocate or panic.
    unsafe impl GlobalAlloc for InspectingAllocator {
        ...
        unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
            if WATCHED_BUFFER
                ...
                .is_ok()
            {
                let wiped = layout.size() == 2 * 1024 * 1024
                    && unsafe { std::slice::from_raw_parts(ptr, layout.size()) }
                        .iter()
                        .all(|&byte| byte == 0);
                BUFFER_WAS_WIPED.store(wiped, Ordering::SeqCst);
            }
    ...
    #[global_allocator]
    static TEST_ALLOCATOR: InspectingAllocator = InspectingAllocator;

    #[test]
    fn inflater_wipes_plaintext_buffer_on_success_and_errors() {
        use flate2::write::ZlibEncoder;
        use flate2::Compression;

        let plaintext = b"secret plaintext that must not survive buffer release";
        ...
        for scenario in 0..3 {
            ...
            if scenario == 1 {
                *bytes.last_mut().unwrap() ^= 1; // Bad zlib checksum.
            } else if scenario == 2 {
                bytes.pop(); // Missing checksum byte, detected by finish().
            }
            let mut inflater = StreamingInflater::new(std::io::sink(), 1024);
            let result = inflater.consume(&bytes);
            assert_eq!(result.is_err(), scenario == 1);
            assert!(inflater.output_buf.iter().any(|&byte| byte != 0));

            BUFFER_WAS_WIPED.store(false, Ordering::SeqCst);
            assert!(WATCHED_BUFFER
                .swap(inflater.output_buf.as_mut_ptr(), Ordering::SeqCst)
                .is_null());
            if scenario == 1 {
                drop(inflater);
            } else {
                assert_eq!(inflater.finish().is_ok(), scenario == 0);
            }
            assert!(WATCHED_BUFFER.load(Ordering::SeqCst).is_null());
            assert!(BUFFER_WAS_WIPED.load(Ordering::SeqCst));
        }
    }
