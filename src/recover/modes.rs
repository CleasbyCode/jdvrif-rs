use crate::common::{
    get_value, open_binary_input_or_throw, span_has_range, StagingFile,
};
use crate::constants::{
    BASE_OFFSET_DEFAULT, CORRUPT_FILE_ERROR, MAX_DATA_SIZE_BLUESKY, MAX_SIZE_CONCEAL,
    NO_ZLIB_COMPRESSION_ID, NO_ZLIB_COMPRESSION_ID_INDEX, STREAM_FRAME_LEN_BYTES,
};
...
pub(super) fn recover_from_icc_path(
    ...
    // The extracted ciphertext is pure scratch, so it goes to a link-free
    // StagingFile. The decrypted payload below cannot: it is committed to its
    // final name by hard link, which requires a real directory entry on the
    // output's filesystem -- so it stays a named 0600 temporary.
    ...
    // Link-free inode: dropping it releases the blocks, nothing to unlink.
    drop(cipher_stage);
