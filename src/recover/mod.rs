...
pub(crate) fn run_native_recover(image_file_path: &Path) -> Result<(), JdvrifError> {
    ...
    // Default: the first ICC-profile signature that also carries the jdvrif
    // signature at +811. One linear pass verifies each candidate in place (see
    // find_signature_pair_in_file), so a file seeded with decoy ICC signatures
    // cannot force a rescan per hit.
