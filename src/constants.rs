...
──────────────────────────
  ...
  jdvrif-rs conceal [-b|-r|-x] <cover_image> <secret_file>
...
so the cover image and the compressed data file share one combined budget:
	...
	• Compressed data file on its own:    ~171 KiB
...
-x, the existing X-Twitter-compatible ICC path and its ~10 KiB first-segment limit remain in use.
...
Platforms with small size limits, like X-Twitter (~10 KiB), are best suited for data that
...
bytes, and the compressed data file alone must not exceed ~171 KiB.
...
Use "jdvrif-rs capsize -r my_image.jpg" to inspect a Reddit cover before choosing a payload.
...
Use "jdvrif-rs capsize -x my_image.jpg" to inspect an X-Twitter cover before choosing a payload.
...
Do not target the exact theoretical limit; when capacity permits, leave at least 1 KiB of margin.
...
pub(crate) const TWITTER_UPLOAD_SIZE_LIMIT: usize = 5 * 1024 * 1024;
