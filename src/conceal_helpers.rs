use crate::common::{
    checked_file_size, has_file_extension, has_safe_embedded_filename, open_binary_input_or_throw,
...
pub(crate) fn temp_compressed_path(base_name: &[u8]) -> Result<PathBuf, String> {
    // Temp filenames are internal and carry a random token, so a lossy stem here
    // is only cosmetic and never affects the embedded (raw-byte) filename.
    let lossy = String::from_utf8_lossy(base_name);
    let stem = Path::new(lossy.as_ref())
        .file_stem()
...
pub(crate) fn temp_encrypted_path(base_name: &[u8]) -> Result<PathBuf, String> {
    let lossy = String::from_utf8_lossy(base_name);
    let stem = Path::new(lossy.as_ref())
        .file_stem()
        .map(|v| v.to_string_lossy().into_owned())
