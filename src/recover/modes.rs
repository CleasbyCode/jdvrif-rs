...
use crate::constants::{
    ...
    STREAM_FRAME_LEN_BYTES, WRITE_COMPLETE_ERROR,
};
...
pub(super) fn recover_from_reddit_path(
    ...
    let embedded_file_size = envelope.encrypted_data.len();
    validate_declared_cipher_size(embedded_file_size, RecoveryFormat::Reddit)?;
    ...
    let prepared = prepare_decrypt_from_kdf_metadata(&envelope.kdf_metadata, recovery_pin)?;
    ...
    output
        .write_all(&envelope.encrypted_data)
