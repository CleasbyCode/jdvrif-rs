...
use crate::conceal::{run_capacity_check, run_native_conceal};
...
fn run() -> ExitCode {
    ...
    match parsed {
        ...
        ParsedCommand::Capsize { option, image } => {
            let image_path = PathBuf::from(image);
            match run_capacity_check(&option, &image_path) {
                Ok(()) => ExitCode::from(0),
                Err(err) => {
                    eprintln!("\n{}\n", err);
                    ExitCode::from(1)
...
mod tests {
    use super::*;
    use crate::common::StagingFile;
    use crate::constants::{
        stream_mode_byte, KDF_ALG_ARGON2ID13, KDF_ALG_OFFSET, KDF_COST_FIELD_BYTES,
        KDF_MAGIC_OFFSET, KDF_MEMLIMIT_OFFSET, KDF_METADATA_MAGIC_V2, KDF_METADATA_MAGIC_V3,
        KDF_METADATA_MAGIC_V4, KDF_NONCE_OFFSET, KDF_OPSLIMIT_OFFSET, KDF_SALT_OFFSET,
    ...
    fn build_metadata_and_ciphertext(
        ...
        let key_bytes = derive_key_from_pin(&pin, &salt, KDF_PARAMS_CURRENT).expect("derive key");
        ...
        let (metadata_magic, associated_data): (&[u8; 4], Option<&[u8]>) = match metadata_version {
            ...
            KdfMetadataVersion::V4RecordedKdfParameters => {
                (&KDF_METADATA_MAGIC_V4, Some(mode_data.as_slice()))
            }
        ...
        if metadata_version == KdfMetadataVersion::V4RecordedKdfParameters {
            let ops = u32::try_from(KDF_PARAMS_CURRENT.opslimit).expect("opslimit fits");
            let mem = u32::try_from(KDF_PARAMS_CURRENT.memlimit).expect("memlimit fits");
            metadata[offsets.sodium_key_index + KDF_OPSLIMIT_OFFSET
                ..offsets.sodium_key_index + KDF_OPSLIMIT_OFFSET + KDF_COST_FIELD_BYTES]
                .copy_from_slice(&ops.to_be_bytes());
            metadata[offsets.sodium_key_index + KDF_MEMLIMIT_OFFSET
                ..offsets.sodium_key_index + KDF_MEMLIMIT_OFFSET + KDF_COST_FIELD_BYTES]
                .copy_from_slice(&mem.to_be_bytes());
        }

        (metadata, ciphertext, payload.to_vec())
    }

    /// The decrypt writer targets an existing link-free staging inode (a
    /// `StagingFile` in the real recover path), so tests stage into one too
    /// rather than handing it a name that does not exist yet.
    fn stage_in(dir: &std::path::Path) -> StagingFile {
        StagingFile::new(dir, "test").expect("staging file")
    }
    ...
    #[test]
    fn decrypt_v4_recorded_kdf_parameters_roundtrip() {
        crate::crypto::init().expect("sodium init");

        let pin = 9_114_552_003_771u64;
        let filename = "v4_payload.bin";
        let expected_payload = b"v4 recorded-parameter payload ".repeat(4096);
        let (metadata, ciphertext, expected_plain) = build_metadata_and_ciphertext(
            false,
            pin,
            filename,
        ...
        let temp = TempDirGuard::new("v4_compressed");
        let cipher_path = temp.path.join("cipher.bin");
        let stage = stage_in(&temp.path);
        let stage_path = stage.path().to_path_buf();
        fs::write(&cipher_path, &ciphertext).expect("write cipher");
    ...
    /// V4 records the Argon2id costs in the image. Out-of-range values drive an
    /// allocation and a work loop that both run before a wrong PIN can be
    /// detected, so they must be refused outright rather than handed to the KDF.
    #[test]
    fn v4_out_of_range_kdf_costs_are_rejected() {
        crate::crypto::init().expect("sodium init");

        let pin = 5_552_119_887_003u64;
        let (metadata, ciphertext, _) = build_metadata_and_ciphertext(
            false,
            pin,
            "v4_costs.bin",
            b"v4 cost bounds payload",
            true,
            KdfMetadataVersion::V4RecordedKdfParameters,
        );
        ...
        let cases: [(usize, u32); 4] = [
            (KDF_OPSLIMIT_OFFSET, 0),
            (KDF_OPSLIMIT_OFFSET, u32::MAX),
            (KDF_MEMLIMIT_OFFSET, 0),
            (KDF_MEMLIMIT_OFFSET, u32::MAX),
        ];

        for (field_offset, value) in cases {
            let mut patched = metadata.clone();
            let at = offsets.sodium_key_index + field_offset;
            patched[at..at + KDF_COST_FIELD_BYTES].copy_from_slice(&value.to_be_bytes());

            let temp = TempDirGuard::new("v4_costs");
            let cipher_path = temp.path.join("cipher.bin");
            let stage = stage_in(&temp.path);
            let stage_path = stage.path().to_path_buf();
            fs::write(&cipher_path, &ciphertext).expect("write cipher");

            // Matched rather than `expect_err`: that would need `Debug` on
            // DecryptStatus, which carries the decrypted filename.
            match decrypt_from_cipher_stage_with_pin(
                &patched,
                false,
                true,
                &cipher_path,
                ...
                Err(err) => assert!(
                    format!("{err:?}").contains("unsupported key-derivation parameters"),
                    "unexpected error for {field_offset:#x}={value}: {err:?}"
                ),
                Ok(_) => {
                    panic!("out-of-range KDF costs were accepted for {field_offset:#x}={value}")
                }
            }
        }
    }
