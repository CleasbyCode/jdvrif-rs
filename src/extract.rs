use crate::common::{
    finish_buffered_file, open_binary_input_or_throw, open_binary_output_for_staging_or_throw,
};
use crate::constants::{
...
/// Largest ciphertext the Bluesky packer can actually hold (EXIF + two Photoshop
/// datasets + the base64 XMP overflow). Conceal checks the encrypted payload
/// against this so an over-large one is refused up front instead of failing deep
/// inside the packer; recovery enforces the same bound below.
pub(crate) fn max_bluesky_embedded_cipher_capacity() -> Result<usize, String> {
...
pub(crate) fn extract_default_ciphertext_to_file(
    ...
    // output_path is the recover cipher StagingFile: a link-free inode that
    // already exists, so exclusive create is neither possible nor needed.
    ...
    while cursor < embedded_file_size {
        if has_profile_headers && cursor == next_header {
            ...
            cursor += skip;
            // Overflow here would leave next_header behind the cursor and make
            // the run length below underflow, so treat it as corruption rather
            // than silently keeping the stale value (C++ uses checkedAdd).
            next_header = next_header
                .checked_add(COMMON_DIFF_VAL)
                .ok_or_else(|| CORRUPT_FILE_ERROR.to_string())?;
            continue;
        }
        ...
        // Unreachable while PROFILE_HEADER_LENGTH > 0: cursor == next_header is
        // claimed by the skip branch above, so this run is always non-empty.
        // Erroring rather than looping means a future layout change cannot turn
        // this into a non-terminating loop driven by attacker-supplied segment
        // counts, and it rules out the underflow if next_cut ever trails cursor.
        let run = next_cut
            .checked_sub(cursor)
            .filter(|r| *r != 0)
            .ok_or_else(|| CORRUPT_FILE_ERROR.to_string())?;
    ...
    // Ciphertext stage file: no fsync needed (it never outlives the run), but
    // the close is checked so a deferred write error cannot masquerade as a
    // complete extraction.
    finish_buffered_file(output, WRITE_COMPLETE_ERROR, false)?;
...
mod tests {
    use super::*;
    // These fixtures create brand-new named files, so they use the exclusive
    // O_EXCL opener rather than the staging opener the extract paths now use.
    use crate::common::{create_new_owner_only, StagingFile};
    use std::time::{SystemTime, UNIX_EPOCH};

    // The extract paths write into the recover cipher stage, a link-free inode
    // that already exists. Tests use a real StagingFile so they exercise the
    // same opener production does.
    fn staging_out() -> StagingFile {
        StagingFile::new(std::path::Path::new(""), "test_extract").expect("staging file")
    }
    ...
    fn base64_stream_decode_roundtrip_size() {
        ...
        let output_file = create_new_owner_only(&out).unwrap();
        let mut output = BufWriter::new(output_file);
    ...
    fn bluesky_extraction_preserves_valid_single_and_split_layouts() {
        ...
        let single_out_stage = staging_out();
        let single_out = single_out_stage.path();
        assert_eq!(
        ...
        let split_out_stage = staging_out();
        let split_out = split_out_stage.path();
        ...
        // Only the named fixtures need removing; the staging outputs are
        // link-free inodes reclaimed when their StagingFile drops.
        for path in [single_path, split_path] {
            let _ = std::fs::remove_file(path);
        }
    }
    ...
    fn bluesky_extraction_preserves_valid_xmp_overflow_layout() {
        ...
        let output_stage = staging_out();
        let output_path = output_stage.path();
        assert_eq!(
            extract_bluesky_ciphertext_to_file(
                &image_path,
                image.len(),
