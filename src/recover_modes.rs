...
use crate::constants::{CORRUPT_FILE_ERROR, NO_ZLIB_COMPRESSION_ID, NO_ZLIB_COMPRESSION_ID_INDEX};
use crate::decrypt::decrypt_from_cipher_stage;
use crate::extract::{
    extract_bluesky_ciphertext_to_file, extract_default_ciphertext_to_file, read_exact_at,
};
use crate::runtime::{DecryptStatus, NativeRecoverError, TempFileGuard};
use std::path::Path;
...
pub(super) fn recover_from_icc_path(
    ...
    let cipher_stage_path = temp_recovery_path(Path::new("jdvrif_cipher.bin"))
        .map_err(NativeRecoverError::Message)?;
    ...
    let decrypt_result = decrypt_from_cipher_stage(
        &metadata,
        false,
        is_data_compressed,
        &cipher_stage_path,
...
pub(super) fn recover_from_bluesky_path(
    ...
    let decrypt_result =
        decrypt_from_cipher_stage(&metadata, true, true, &cipher_stage_path, &stream_stage_path)?;
