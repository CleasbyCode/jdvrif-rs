...
fn print_carrier_capacity_limits(
    ...
    println!(
        "{theoretical_label:<71}{theoretical_capacity} bytes (~{}KiB).",
...
fn fail_reddit_payload_capacity(
    ...
    println!(
        "Compressed data file (payload) size: {stored_payload_size} bytes ({}KiB).\n",
    ...
    Err(JdvrifError::new(format!(
        "Data File Size Error: \n\nAlready-compressed payload file size of {stored_payload_size} bytes ({}KiB) exceeds the recommended maximum limit of {} bytes (~{}KiB) for this cover image.",
        stored_payload_size / BYTES_PER_KIBIBYTE,
        limits.recommended_compressed,
        limits.recommended_compressed / BYTES_PER_KIBIBYTE,
    )))
...
The final embedded JPEG size can differ from the cover image and must remain within 20MiB.\n"
    );
    Ok(())
}
...
The final embedded JPEG size can differ from the cover image and must remain within 5MiB.\n"
    );
    Ok(())
}
...
pub(crate) fn run_native_conceal(
    ...
    // >10 MB source, which can never fit the ~171 KiB Bluesky payload limit
