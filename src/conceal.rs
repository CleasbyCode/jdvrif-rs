...
use crate::conceal_helpers::{
    ...
    platform_report_template, print_conceal_summary, should_bypass_compression,
    temp_compressed_path, temp_encrypted_path, validate_combined_size_limits,
...
use crate::constants::{
    BLUESKY_KDF_METADATA_INDEX, BLUESKY_PLATFORM_INDEX, DEFAULT_KDF_METADATA_INDEX,
    KDF_ALG_ARGON2ID13, KDF_ALG_OFFSET, KDF_MAGIC_OFFSET, KDF_METADATA_MAGIC_V2,
    KDF_METADATA_REGION_BYTES, KDF_NONCE_OFFSET, KDF_SALT_OFFSET, KDF_SENTINEL,
    KDF_SENTINEL_OFFSET, NO_ZLIB_COMPRESSION_ID, NO_ZLIB_COMPRESSION_ID_INDEX, REDDIT_PLATFORM_INDEX,
...
pub(crate) fn run_native_conceal(
    ...
    crate::crypto::init().map_err(|_| "Libsodium initialization failed!".to_string())?;
    ...
    if !span_has_range(segment_vec.len(), kdf_metadata_index, KDF_METADATA_REGION_BYTES) {
        return Err("Internal Error: Corrupt segment metadata template.".to_string());
    }

    // Generate key material
    let recovery_pin = generate_recovery_pin();
    let mut salt = [0u8; argon2id13::SALTBYTES];
    randombytes_into(&mut salt);
    let mut key_bytes = derive_key_from_pin(recovery_pin, &salt)?;
    ...
    if has_bluesky_option {
        ...
        let mut bluesky_builder = BlueskySegmentStreamBuilder::new(&mut segment_vec, encrypted_size)?;
        ...
        let output_path = crate::output_file::write_to_staged_output(
            std::path::Path::new(""),
            "jrif_",
            ".jpg",
            9,
            ...
            |out| {
                if !segment_vec.is_empty() {
                    out.write(&segment_vec, "Write File Error: Output data too large to write.")?;
                }
                out.write(&jpg_vec, "Write File Error: Output data too large to write.")?;
                Ok(())
            },
        )?;

        print_conceal_summary(&platforms_vec, &output_path, embedded_jpg_size, recovery_pin);
        return Ok(());
    }

    // ── Default / Reddit path: encrypt to temp file, then stream via OutputFile ──
    let enc_tmp = temp_encrypted_path(&data_filename)?;
    ...
    let output_path = crate::output_file::write_to_staged_output(
        ...
        |out| {
            summary =
                write_embedded_jpg_from_encrypted_file(out, &mut segment_vec, &enc_tmp, &jpg_vec, has_reddit_option)?;
            Ok(())
        },
    )?;
    ...
    print_conceal_summary(&platforms_vec, &output_path, summary.embedded_image_size, recovery_pin);
    Ok(())
}
