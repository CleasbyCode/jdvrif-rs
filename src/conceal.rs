...
use crate::common::{
    checked_file_size, cleanup_path_no_throw, finish_buffered_file, open_binary_input_or_throw,
...
pub(crate) fn run_native_conceal(
    ...
    let (encrypt_input_path, encrypt_input_size) = if bypass_compression {
        ...
        let compressed_path = temp_compressed_path().map_err(JdvrifError::from)?;
        zlib_compress_file_to_path_native(data_file_path, &compressed_path, source_data_size)
            .map_err(JdvrifError::from)?;
        // Take ownership of the temp before anything that can fail, so a failure
        // in the size probe below cannot orphan it in the working directory.
    ...
    // ── Default path: encrypt to temp file, then stream via OutputFile ──
    let enc_tmp = temp_encrypted_path().map_err(JdvrifError::from)?;
    ...
    let staged_output = crate::output_file::write_to_uncommitted_staged_output(
        ...
        |out| {
            summary =
                write_embedded_jpg_from_encrypted_file(out, &mut segment_vec, &enc_tmp, &jpg_vec)?;
            Ok(())
        },
    )
    .map_err(JdvrifError::from)?;
