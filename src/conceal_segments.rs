...
use crate::constants::{
    ...
    DEFLATED_DATA_FILE_SIZE_INDEX, ICC_PROFILE_SIG, JDVRIF_SIG, MAX_SIZE_REDDIT_PADDING,
    PADDING_RANGE, PADDING_START, PHOTOSHOP_SEGMENT_TEMPLATE, PROFILE_DATA_SIZE, PROFILE_SIZE_DIFF,
    PROFILE_SIZE_INDEX, SEGMENTS_TOTAL_VAL_INDEX, SEGMENT_DATA_SIZE, SEGMENT_HEADER_LENGTH,
    SEGMENT_HEADER_SIZE_INDEX, SEGMENT_SIG_LENGTH, SOI_SIG_LENGTH, XMP_SEGMENT_TEMPLATE,
};
...
fn write_reddit_padding_to_output(out: &mut OutputFile) -> Result<(), String> {
    ...
    while remaining > 0 {
        ...
        for b in chunk[..batch].iter_mut() {
            let r = randombytes_uniform(PADDING_RANGE)
                .map_err(|_| "CSPRNG Error: Failed to generate random padding.".to_string())?;
            *b = PADDING_START + r as u8;
        }
