...
use kdf::{
    derive_secretstream_key_and_header, read_kdf_params, require_reddit_secretstream,
...
/// Takes the PIN the caller has already collected: the Reddit carrier's
/// coefficient positions derive from it, so recover must obtain it before it can
/// locate anything, and must not prompt a second time here.
pub(crate) fn prepare_decrypt_from_kdf_metadata(
    ...
    let metadata_version = require_reddit_secretstream(metadata, offsets)?;
