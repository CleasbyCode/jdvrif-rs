...
fn usage_message(prog: &str) -> String {
    format!(
        "Usage: {prog} conceal [-b|-r|-x] <cover_image> <secret_file>\n       {prog} recover <cover_image>\n       {prog} capsize [-r|-x] <cover_image>\n       {prog} --info"
    )
}
...
pub(crate) fn parse_args(args: &[OsString]) -> Result<ParsedCommand, String> {
    ...
    if args[0] == OsStr::new("conceal") {
        let mut i = 1usize;
        let option = if i < args.len() {
            if args[i] == OsStr::new("-b") {
                i += 1;
                ConcealOption::Bluesky
            } else if args[i] == OsStr::new("-r") {
                i += 1;
                ConcealOption::Reddit
            } else if args[i] == OsStr::new("-x") {
                i += 1;
    ...
    if args[0] == OsStr::new("capsize") {
        let mut i = 1usize;
        let option = if i < args.len() && args[i] == OsStr::new("-r") {
            i += 1;
            ConcealOption::Reddit
        } else {
            if i < args.len() && args[i] == OsStr::new("-x") {
                i += 1;
            }
            ConcealOption::Twitter
        };
        ...
        return Ok(ParsedCommand::Capsize {
            option,
            image: args[i].clone(),
        });
    }
...
mod tests {
    ...
    #[test]
    fn parse_capsize_options() {
        match parse_args(&[os("capsize"), os("a.jpg")]).unwrap() {
            ParsedCommand::Capsize {
                option: ConcealOption::Twitter,
                image,
            } => assert_eq!(image, os("a.jpg")),
            _ => panic!("default capsize"),
        }
        match parse_args(&[os("capsize"), os("-r"), os("a.jpg")]).unwrap() {
            ParsedCommand::Capsize {
                option: ConcealOption::Reddit,
                image,
            } => assert_eq!(image, os("a.jpg")),
            _ => panic!("reddit capsize"),
        }
        match parse_args(&[os("capsize"), os("-x"), os("a.jpg")]).unwrap() {
            ParsedCommand::Capsize {
                option: ConcealOption::Twitter,
                image,
            } => assert_eq!(image, os("a.jpg")),
            _ => panic!("twitter capsize"),
        }
    }
    ...
    fn parse_conceal_options() {
        ...
        match parse_args(&[os("conceal"), os("-x"), os("a.jpg"), os("s.bin")]).unwrap() {
            ParsedCommand::Conceal {
                option: ConcealOption::Twitter,
                ..
            } => {}
    ...
    fn parse_rejects_bad_arity() {
        ...
        assert!(parse_args(&[os("capsize")]).is_err());
        assert!(parse_args(&[os("capsize"), os("-b"), os("a.jpg")]).is_err());
