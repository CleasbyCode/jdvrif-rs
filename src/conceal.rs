...
use crate::common::{
    ...
    open_binary_output_for_staging_or_throw, span_has_range, validate_file_for_read, StagingFile,
};
...
pub(crate) fn run_native_conceal(
    ...
    let (encrypt_input_path, encrypt_input_size) = if bypass_compression {
        ...
        let stage = StagingFile::new(std::path::Path::new(""), "comp").map_err(JdvrifError::from)?;
    ...
    let enc_stage =
        StagingFile::new(std::path::Path::new(""), "enc").map_err(JdvrifError::from)?;
