...
pub(crate) fn validate_combined_size_limits(
    ...
    // segments hold ~171 KiB, well under MAX_DATA_SIZE_BLUESKY, and anything
...
const PIN_DELIVERY_ERROR: &str = "Output Error: Failed to deliver recovery PIN.";
...
pub(crate) fn deliver_conceal_pin(
    ...
    // `write!("{}", pin)` would stage the PIN's decimal form in a heap buffer
    // that is freed without being zeroed, undoing the wiping the rest of the
    // PIN path is careful about. Render the digits into a stack array that is
    // zeroed on every exit path (Zeroizing), and emit them between two constant
    // fragments instead.
    let mut digits = Zeroizing::new([0u8; 20]);
    let decimal_len = render_u64_decimal(recovery_pin, &mut digits[..]);
    output
        .write_all(b"\nRecovery PIN: [***")
        .map_err(|_| PIN_DELIVERY_ERROR.to_string())?;
    output
        .write_all(&digits[..decimal_len])
        .map_err(|_| PIN_DELIVERY_ERROR.to_string())?;
    output
        .write_all(
        ...
        .map_err(|_| PIN_DELIVERY_ERROR.to_string())?;
    output.flush().map_err(|_| PIN_DELIVERY_ERROR.to_string())?;
    Ok(())
}

/// Write `value`'s decimal digits into the tail-aligned start of `out`, then
/// shift them to the front. Returns the digit count. No allocation, so the
/// caller's buffer is the only place the PIN's text form ever exists.
fn render_u64_decimal(value: u64, out: &mut [u8]) -> usize {
    ...
    out[..len].copy_from_slice(&scratch[i..]);
    // `scratch` held the digits too; clear it before the frame goes away.
    scratch.zeroize();
    len
}
...
pub(crate) fn is_already_compressed_file_type(data_file_path: &Path) -> bool {
    has_file_extension(
        data_file_path,
        &[
...
pub(crate) fn should_bypass_compression(data_file_path: &Path, source_data_size: usize) -> bool {
    source_data_size > COMPRESS_BYPASS_SIZE && is_already_compressed_file_type(data_file_path)
}

/// Two tiers. The "maximum compression" tier that used to sit here is not worth
/// paying for: measured on a 150 MB mixed-binary corpus, zlib level 9 took
/// 19.4 s for 58,100,237 bytes against 6.3 s for 58,360,140 at level 6 -- 3.1x
/// the time for 0.45% smaller output. Very large inputs drop to fastest so a
/// conceal stays bounded in time.
///
/// The C++ reference additionally caps a whole-buffer libdeflate fast path at
/// 128 MiB (peak RSS and an uninterruptible window). This port has no such path:
/// `zlib_compress_file_to_path_native` always streams in 2 MiB chunks and checks
/// for cancellation every chunk, so neither bound has an analogue here.
...
pub(crate) fn zlib_compress_file_to_path_native(
    ...
    // Deflate is not encryption, and this window holds the payload as raw
    // plaintext, so it is zeroed on every path out rather than merely dropped --
    // matching the WipeBytesGuard the C++ reference puts on its in_chunk. (The
    // compressed side has no analogue here: flate2 and BufWriter own those
    // buffers internally, where the C++ out_chunk is ours to wipe.)
    let mut in_chunk = Zeroizing::new(vec![0u8; 2 * 1024 * 1024]);
...
mod tests {
    ...
    fn compression_rejects_growth_beyond_validated_size() {
        ...
        let stage =
            crate::common::StagingFile::new(Path::new(""), "test_comp").expect("staging file");
