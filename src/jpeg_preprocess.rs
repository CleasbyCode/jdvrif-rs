...
pub(crate) const MAX_BLUESKY_IMAGE_SIZE: usize = 2_000_000;
...
const DEFAULT_COVER_IMAGE_LIMITS: CoverImageLimits = CoverImageLimits {
    max_dimension: 4_096,
    max_pixels: 4_096 * 4_096,
};
const REDDIT_COVER_IMAGE_LIMITS: CoverImageLimits = CoverImageLimits {
    max_dimension: 8_192,
    max_pixels: 8_192 * 8_192,
};
...
// turbojpeg.h: limit excessive progressive scans during opaque transforms.
const TJFLAG_LIMITSCANS: i32 = 32768;
...
/// The N field bytes at `offset`, bounds-checked. TIFF fields are stored in the
/// byte order the header declares, so the callers below pick the decoding.
fn tiff_field_bytes<const N: usize>(tiff: &[u8], offset: usize) -> Option<[u8; N]> {
    if !span_has_range(tiff.len(), offset, N) {
        return None;
    }
    tiff[offset..offset + N].try_into().ok()
}

fn read_tiff_u16(tiff: &[u8], offset: usize, little_endian: bool) -> Option<u16> {
    let raw = tiff_field_bytes::<2>(tiff, offset)?;
...
fn read_tiff_u32(tiff: &[u8], offset: usize, little_endian: bool) -> Option<u32> {
    let raw = tiff_field_bytes::<4>(tiff, offset)?;
    Some(if little_endian {
...
/// Owns the transformed JPEG exactly as TurboJPEG produced it. `offset` drops
/// the leading markers that conceal replaces with its own segment template, so
/// the trim is a view rather than a second full-image copy.
pub struct OptimizedCover {
    buffer: TurboJpegBuffer,
    offset: usize,
    size: usize,
}

impl std::fmt::Debug for OptimizedCover {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // Sizes only -- never the image bytes.
        f.debug_struct("OptimizedCover")
...
impl OptimizedCover {
    pub fn view(&self) -> &[u8] {
        if self.buffer.raw.is_null() || self.size == 0 {
            return &[];
        }
        // SAFETY: buffer owns a TurboJPEG allocation of at least offset+size
        // bytes (checked against dst_len before construction) and outlives the
        // borrow.
        unsafe { std::slice::from_raw_parts(self.buffer.raw.add(self.offset), self.size) }
    }
    ...
    pub fn full_view(&self) -> &[u8] {
        ...
        // SAFETY: buffer owns exactly offset+size bytes and outlives the borrow.
        unsafe { std::slice::from_raw_parts(self.buffer.raw, self.offset + self.size) }
    }
}
...
fn optimize_image_with_turbojpeg(
    jpg_vec: &mut Vec<u8>,
    is_progressive: bool,
    enforce_quality_limit: bool,
    limits: CoverImageLimits,
) -> Result<OptimizedCover, String> {
    check_cancellation()?;
    if jpg_vec.is_empty() {
    ...
    if prescan.width > limits.max_dimension
        || prescan.height > limits.max_dimension
    ...
    let transformed = {
        // SAFETY: dst_guard owns a valid TurboJPEG-allocated buffer of dst_len
        // bytes; the borrow ends with this block, before the guard is moved.
        let result = unsafe { std::slice::from_raw_parts(dst_guard.raw, dst_len) };
        inspect_transformed_jpeg(result)
    };
    ...
    if enforce_quality_limit && estimated_quality > MAX_ALLOWED_QUALITY {
        return Err(format!(
    ...
    // first DQT are skipped by the offset rather than copied away. The source
    // image is no longer needed, so release it here.
    jpg_vec.clear();
    jpg_vec.shrink_to_fit();
    Ok(OptimizedCover {
        buffer: dst_guard,
        offset: transformed.offset,
        size: dst_len - transformed.offset,
    })
}
...
pub fn prepare_cover_image_for_conceal(
    ...
    let cover = optimize_image_with_turbojpeg(
        &mut jpg_vec,
        is_progressive,
        has_no_option,
        DEFAULT_COVER_IMAGE_LIMITS,
    )?;

    if cover.trimmed_size() > MAX_OPTIMIZED_IMAGE_SIZE {
        return Err(
    ...
    if has_bluesky_option && cover.trimmed_size() > MAX_BLUESKY_IMAGE_SIZE {
        return Err(
...
pub(crate) fn prepare_cover_image_for_reddit(
    image_file_path: &Path,
    expected_image_size: usize,
) -> Result<OptimizedCover, String> {
    let mut input = open_binary_input_or_throw(
    ...
    // Preserve the reference path's lossless EXIF-orientation transform and
    // metadata stripping before the Reddit pixel transcode. Reddit replaces
    // default conceal's 4 MiB optimized-cover cap with its own final 20 MiB cap.
    optimize_image_with_turbojpeg(&mut jpg_vec, false, false, REDDIT_COVER_IMAGE_LIMITS)
}

pub(crate) fn prepare_cover_image_for_twitter(
    image_file_path: &Path,
    expected_image_size: usize,
) -> Result<OptimizedCover, String> {
    let mut input = open_binary_input_or_throw(
    ...
    // Match the reference path's cheap header/dimension refusal before the
    // orientation transform and progressive pixel transcode.
    crate::twitter::inspect_cover(&jpg_vec)?;
    optimize_image_with_turbojpeg(&mut jpg_vec, false, false, DEFAULT_COVER_IMAGE_LIMITS)
}
...
mod tests {
    ...
    #[test]
    fn progressive_scan_exhaustion_is_rejected() {
        let source = include_bytes!("testdata/twitter_low_amplitude.jpg");
        let last_scan = source
            .windows(2)
            .rposition(|marker| marker == [0xff, 0xda])
            .expect("progressive fixture contains a scan");
        assert_eq!(&source[source.len() - 2..], &[0xff, 0xd9]);
        let mut normal = source.to_vec();
        optimize_image_with_turbojpeg(&mut normal, true, false, DEFAULT_COVER_IMAGE_LIMITS)
            .expect("ordinary progressive fixture transforms");

        // Repeating a complete late scan adds very little input while forcing
        // the decoder to revisit the coefficients on every pass.
        let mut excessive = source[..source.len() - 2].to_vec();
        for _ in 0..600 {
            excessive.extend_from_slice(&source[last_scan..source.len() - 2]);
        }
        excessive.extend_from_slice(&[0xff, 0xd9]);
        let error = optimize_image_with_turbojpeg(
            &mut excessive,
            true,
            false,
            DEFAULT_COVER_IMAGE_LIMITS,
        ...
        .err()
        .expect("excessive progressive scans must fail");
        assert!(error.to_ascii_lowercase().contains("scan"), "{error}");
    }
    ...
    #[test]
    fn default_cover_dimensions_stop_at_4096() {
        assert_eq!(DEFAULT_COVER_IMAGE_LIMITS.max_dimension, 4_096);
        assert_eq!(DEFAULT_COVER_IMAGE_LIMITS.max_pixels, 4_096 * 4_096);
        assert_eq!(REDDIT_COVER_IMAGE_LIMITS.max_dimension, 8_192);
        assert_eq!(REDDIT_COVER_IMAGE_LIMITS.max_pixels, 8_192 * 8_192);

        let mut jpg = vec![0xFF, 0xD8];
        append_segment(&mut jpg, 0xC0, &sof_payload(4_097, 960, 0));
        let error =
            optimize_image_with_turbojpeg(&mut jpg, false, false, DEFAULT_COVER_IMAGE_LIMITS)
                .expect_err("a 4097-pixel side must be rejected before decoding");
        assert_eq!(
            error,
            "Image Error: Dimensions 4097x960 exceed the safe cover-image limit (4096px per dimension and 16777216 total pixels)."
        );
    }
}
