use crate::common::{
    checked_file_size, get_value, open_binary_input_or_throw, span_has_range, update_value,
};
use crate::constants::{
    ...
    DEFLATED_DATA_FILE_SIZE_INDEX, ICC_PROFILE_SIG, JDVRIF_SIG, PHOTOSHOP_SEGMENT_TEMPLATE,
    PROFILE_DATA_SIZE, PROFILE_SIZE_DIFF, PROFILE_SIZE_INDEX, SEGMENTS_TOTAL_VAL_INDEX,
    SEGMENT_DATA_SIZE, SEGMENT_HEADER_LENGTH, SEGMENT_HEADER_SIZE_INDEX, SEGMENT_SIG_LENGTH,
...
pub(crate) fn write_icc_data_to_output(
    ...
    // Each segment is an 18-byte ICC header followed by its data portion, which
    // `copy_to` streams straight from the encrypted file via sendfile(2) -- the
    // payload never passes through user space. Headers do not coalesce across
    // segments: each `send_from` drains the buffer to keep byte order, so the
    // steady state is one small write(2) plus one sendfile(2) per 64 KiB
    // segment. That is already the floor for interleaved header/payload output
    // without giving up the zero-copy payload path.
...
/// Port of C++ writeEmbeddedJpgFromEncryptedFile.
pub(crate) fn write_embedded_jpg_from_encrypted_file(
    ...
    let mut summary = write_icc_data_to_output(out, segment_vec, encrypted_fd, encrypted_size)?;
...
fn binary_to_base64_finalize(out: &mut Vec<u8>, carry: &[u8; 3], carry_len: usize) {
    // Tail of 1 or 2 bytes: the missing input byte(s) contribute zero bits and
    // are represented by '=' in the output quartet.
    if carry_len == 0 {
        return;
    }
    let a = carry[0];
    let b = if carry_len == 2 { carry[1] } else { 0 };
    out.push(BASE64_TABLE[(a >> 2) as usize]);
    out.push(BASE64_TABLE[(((a & 0x03) << 4) | (b >> 4)) as usize]);
    out.push(if carry_len == 2 {
        BASE64_TABLE[((b & 0x0F) << 2) as usize]
    } else {
        b'='
    });
    out.push(b'=');
...
impl<'a> BlueskySegmentStreamBuilder<'a> {
    pub(crate) fn new(segment_vec: &'a mut Vec<u8>, encrypted_size: usize) -> Result<Self, String> {
        ...
        } else if get_value(segment_vec, BLUESKY_EXIF_SEGMENT_SIZE_INDEX, 2)?
            != exif_segment_data_size
            || get_value(
                segment_vec,
                BLUESKY_ARTIST_FIELD_SIZE_INDEX,
        ...
        {
            // Overflow path: the EXIF payload is always packed to
            // BSKY_EXIF_SEGMENT_DATA_SIZE_LIMIT, so the segment is at its maximum
            // size and the template already carries those values pre-filled.
            // Skipping the writes is only correct while that stays true -- verify
            // rather than assume, exactly as the Photoshop segment does below, so
            // a template edit fails loudly instead of silently emitting an EXIF
            // segment whose declared sizes disagree with its contents.
            return Err("Internal Error: Corrupt Bluesky segment template.".to_string());
        }
        ...
        } else if get_value(&pshop_vec, BSKY_FIRST_DATASET_SIZE_INDEX, 2)?
            != layout.first_dataset_size
        {
            // A full first dataset relies on the template already declaring the
            // limit; check it rather than trusting the asset (see `finish`).
            return Err("Internal Error: Corrupt Bluesky Photoshop segment template.".to_string());
        }
    ...
    pub(crate) fn finish(mut self) -> Result<(), String> {
        ...
        if self.layout.has_pshop_segment() && self.pshop_vec.len() > BSKY_PSHOP_VEC_DEFAULT_SIZE {
            ...
            } else if get_value(&self.pshop_vec, BSKY_SEGMENT_SIZE_INDEX, 2)?
                != pshop_segment_data_size
                || get_value(&self.pshop_vec, BSKY_BIM_SECTION_SIZE_INDEX, 2)? != bim_section_size
            {
                // With an XMP overflow segment both Photoshop datasets are packed
                // to their limits, so the segment is always its maximum size and
                // the template already carries those values pre-filled. Skipping
                // the writes is only correct while that stays true -- verify
                // rather than assume, so a template edit fails loudly instead of
                // silently emitting declared sizes that disagree with the contents.
                return Err(
                    "Internal Error: Corrupt Bluesky Photoshop segment template.".to_string(),
                );
            }
...
mod tests {
    ...
    #[test]
    fn bluesky_layout_splits_payload_across_segments() {
        let layout = BlueskyLayout::from_encrypted_size(100);
        assert_eq!(layout.first_exif_size, 100);
        assert_eq!(layout.first_dataset_size, 0);
        assert_eq!(layout.second_dataset_size, 0);
        assert_eq!(layout.xmp_raw_size, 0);
        assert!(!layout.has_xmp_segment());
        assert!(!layout.has_pshop_segment());

        // Over EXIF limit: remainder spills into Photoshop datasets.
        let large = BSKY_EXIF_SEGMENT_DATA_SIZE_LIMIT + 100;
        let layout = BlueskyLayout::from_encrypted_size(large);
        assert_eq!(layout.first_exif_size, BSKY_EXIF_SEGMENT_DATA_SIZE_LIMIT);
        assert_eq!(layout.first_dataset_size, 100);
        assert!(!layout.has_xmp_segment());
        assert!(layout.has_pshop_segment());
    }

    #[test]
    fn base64_streaming_encode_matches_known_vector() {
        let mut out = Vec::new();
        let mut carry = [0u8; 3];
        let mut carry_len = 0usize;
        binary_to_base64_append_streaming(b"Man", &mut out, &mut carry, &mut carry_len);
        binary_to_base64_finalize(&mut out, &carry, carry_len);
        assert_eq!(out, b"TWFu");
    }

    #[test]
    fn base64_streaming_encode_pads_partial_tails() {
        // The 1- and 2-byte tails are the padded quartets; both share one code
        // path with the full triplet, so pin the canonical vectors for each.
        for (input, expected) in [(&b"M"[..], &b"TQ=="[..]), (&b"Ma"[..], &b"TWE="[..])] {
            let mut out = Vec::new();
            let mut carry = [0u8; 3];
            let mut carry_len = 0usize;
            binary_to_base64_append_streaming(input, &mut out, &mut carry, &mut carry_len);
            binary_to_base64_finalize(&mut out, &carry, carry_len);
