...
for source in reddit_steg.h reddit_steg.cpp; do
    if ! cmp -s "$ROOT_DIR/src/$source" "$RUST_PORT_DIR/native/$source"; then
        echo "Parity mismatch: Rust native/$source differs from the current C++ source." >&2
        diff -u "$ROOT_DIR/src/$source" "$RUST_PORT_DIR/native/$source" || true
        exit 1
    fi
done

echo "Parity smoke passed: shared --info content matches (build section + program name intentionally differ)."
echo "Reddit carrier source parity passed: native reddit_steg.{h,cpp} matches C++."
