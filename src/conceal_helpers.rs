use crate::common::{
    ...
    open_binary_input_or_throw, open_binary_output_for_write_or_throw,
};
use crate::constants::{
    BLUESKY_PLATFORM_INDEX, COMPRESS_BYPASS_SIZE, DATA_FILENAME_MAX_LENGTH, LARGE_FILE_SIZE,
    MAX_DATA_SIZE_BLUESKY, MAX_PATH_ATTEMPTS, MAX_SIZE_CONCEAL, WRITE_COMPLETE_ERROR,
};
...
// Neutral prefix only -- the payload stem is deliberately not part of the name.
// A directory listing during conceal would otherwise reveal the secret filename.
pub(crate) fn temp_compressed_path() -> Result<PathBuf, String> {
...
pub(crate) fn temp_encrypted_path() -> Result<PathBuf, String> {
    unique_randomized_path_or_throw(
        Path::new(""),
        ".jdvrif_enc_",
        ".bin",
