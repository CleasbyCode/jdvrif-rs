...
run_wrong_pin_once() {
    ...
    if ! grep -Eq 'Invalid recovery PIN|Invalid PIN, or this is not a valid jdvrif' recover.log; then
...
run_repeated_wrong_pin_no_destroy() {
    ...
    for i in 1 2 3 4; do
        ...
        if ! grep -Eq 'Invalid recovery PIN|Invalid PIN, or this is not a valid jdvrif' "recover_$i.log"; then
...
run_case "twitter_text" "-x" "$FIXTURES_DIR/covers/cover_default.jpg" "$FIXTURES_DIR/payloads/payload_text.txt" 0 -1
