...
use crate::common::{
    ...
    validate_reddit_cover_for_read, validate_twitter_cover_for_read, StagingFile,
};
use crate::conceal_helpers::{
    deliver_conceal_pin, finalize_default_platform_report, generate_recovery_pin,
    is_already_compressed_file_type, maybe_print_large_file_notice, platform_report_template,
    print_conceal_complete, should_bypass_compression, validate_combined_size_limits,
    validate_data_filename, zlib_compress_file_to_path_native,
};
...
use crate::constants::{
    ...
    STREAM_CHUNK_SIZE, STREAM_FRAME_LEN_BYTES, TWITTER_UPLOAD_SIZE_LIMIT,
};
...
struct PreparedPayloadInput {
    path: PathBuf,
    size: usize,
    is_compressed: bool,
    // Keeps a link-free compressed staging inode alive while `path` refers to
    // it through /proc/self/fd. None means the original payload is stored raw.
    _compressed_stage: Option<StagingFile>,
}

fn prepare_payload_input(
    data_file_path: &Path,
    source_data_size: usize,
    bypass_compression: bool,
    staging_tag: &str,
    compression_error: &str,
) -> Result<PreparedPayloadInput, JdvrifError> {
    if bypass_compression {
        return Ok(PreparedPayloadInput {
            path: data_file_path.to_path_buf(),
            size: source_data_size,
            is_compressed: false,
            _compressed_stage: None,
        });
    }

    let stage = StagingFile::new(Path::new(""), staging_tag).map_err(JdvrifError::from)?;
    zlib_compress_file_to_path_native(data_file_path, stage.path(), source_data_size)
        .map_err(JdvrifError::from)?;
    let compressed_size =
        checked_file_size(stage.path(), compression_error, true).map_err(JdvrifError::from)?;
    let path = stage.path().to_path_buf();
    Ok(PreparedPayloadInput {
        path,
        size: compressed_size,
        is_compressed: true,
        _compressed_stage: Some(stage),
    })
}
...
type CarrierEnvelopeSizeFunction = fn(usize) -> Result<usize, String>;

fn max_carrier_stored_payload(
    carrier_capacity: usize,
    filename_length: usize,
    envelope_size: CarrierEnvelopeSizeFunction,
) -> Result<usize, String> {
    let filename_prefix_size = 1usize
        .checked_add(filename_length)
        .ok_or_else(|| "File Size Error: Encrypted carrier output overflow.".to_string())?;
    let fits = |stored_payload_size: usize| -> Result<bool, String> {
        let encrypted_size =
            estimate_prefixed_stream_encrypted_size(stored_payload_size, filename_prefix_size)?;
        Ok(envelope_size(encrypted_size)? <= carrier_capacity)
    };
...
fn carrier_capacity_limits(
    carrier_capacity: usize,
    envelope_size: CarrierEnvelopeSizeFunction,
) -> Result<CarrierCapacityLimits, String> {
    let conservative_compressed = max_carrier_stored_payload(
        carrier_capacity,
        crate::constants::DATA_FILENAME_MAX_LENGTH,
        envelope_size,
    )?;
    Ok(CarrierCapacityLimits {
        conservative_compressed,
        recommended_compressed: conservative_compressed.saturating_sub(1000),
    })
}

fn print_carrier_capacity_limits(
    theoretical_label: &str,
    theoretical_capacity: usize,
    limits: CarrierCapacityLimits,
) {
    println!(
        "{theoretical_label:<71}{theoretical_capacity} bytes (~{}KB).",
...
fn prepare_reddit_carrier(
    image_file_path: &Path,
    cover_size: usize,
) -> Result<reddit::PreparedRedditCover, JdvrifError> {
    let normalized =
...
fn print_reddit_cover_image_summary(cover: &reddit::PreparedRedditCover, source_cover_size: usize) {
    println!(
        "Cover Image: {}KB, {}x{}, Baseline YCbCr 4:2:0, Standard Q75 quantization (C3).\n",
...
fn reddit_capacity_limits(
    cover: &reddit::PreparedRedditCover,
) -> Result<CarrierCapacityLimits, String> {
    carrier_capacity_limits(cover.payload_capacity, reddit::envelope_size)
}

fn print_reddit_capacity_limits(
    cover: &reddit::PreparedRedditCover,
    limits: CarrierCapacityLimits,
) {
    print_carrier_capacity_limits(
...
fn fail_reddit_payload_capacity(
    cover: &reddit::PreparedRedditCover,
    source_cover_size: usize,
    stored_payload_size: usize,
) -> Result<(), JdvrifError> {
    let limits = reddit_capacity_limits(cover).map_err(JdvrifError::from)?;
    println!();
    print_reddit_cover_image_summary(cover, source_cover_size);
    println!(
        "Compressed data file (payload) size: {stored_payload_size} bytes ({}KB).\n",
        stored_payload_size / 1000
    );
    print_reddit_capacity_limits(cover, limits);
    let _ = std::io::stdout().lock().flush();

    Err(JdvrifError::new(format!(
        "Data File Size Error: \n\nAlready-compressed payload file size of {stored_payload_size} bytes ({}KB) exceeds the recommended maximum limit of {} bytes (~{}KB) for this cover image.",
        stored_payload_size / 1000,
        limits.recommended_compressed,
        limits.recommended_compressed / 1000,
    )))
...
fn run_reddit_conceal(
    ...
    let prepared_cover = prepare_reddit_carrier(image_file_path, cover_size)?;

    // A second zlib layer can expand already-compressed formats. The Reddit
    // carrier therefore bypasses compression for recognized types at any size.
    let payload_input = prepare_payload_input(
        data_file_path,
        source_data_size,
        is_already_compressed_file_type(data_file_path),
        "reddit_comp",
        "Zlib Compression Error: Failed to build Reddit compressed payload.",
    )?;
    let stored_payload_size = payload_input.size;
    ...
    let encrypted_size =
        estimate_prefixed_stream_encrypted_size(stored_payload_size, filename_prefix_len)
            .map_err(JdvrifError::from)?;
    ...
    if envelope_size > prepared_cover.payload_capacity {
        return fail_reddit_payload_capacity(&prepared_cover, cover_size, stored_payload_size);
    }
    ...
    let stream_header = encrypt_file_with_secretstream_to_file(
        ...
        stored_payload_size,
        &filename_prefix,
        stream_mode_byte(payload_input.is_compressed),
        &key,
    ...
    let envelope = Zeroizing::new(
        reddit::make_envelope(&kdf_metadata, &encrypted_data, payload_input.is_compressed)
            .map_err(JdvrifError::from)?,
    );
...
fn validate_twitter_preliminary_limits(
    cover_size: usize,
    payload_size: usize,
) -> Result<(), String> {
    let cover_too_large = cover_size > TWITTER_UPLOAD_SIZE_LIMIT;
    let payload_too_large = payload_size > TWITTER_UPLOAD_SIZE_LIMIT;
    if cover_too_large && payload_too_large {
        return Err(
...
fn prepare_twitter_carrier(
    image_file_path: &Path,
    cover_size: usize,
) -> Result<twitter::PreparedTwitterCover, JdvrifError> {
    let normalized =
        crate::jpeg_preprocess::prepare_cover_image_for_twitter(image_file_path, cover_size)
            .map_err(JdvrifError::from)?;
    let prepared_cover =
        twitter::prepare_cover(normalized.full_view()).map_err(JdvrifError::from)?;
    drop(normalized);

    if prepared_cover.prepared_jpeg_size > TWITTER_UPLOAD_SIZE_LIMIT {
        return Err(JdvrifError::new(
...
fn print_twitter_cover_image_summary(
    cover: &twitter::PreparedTwitterCover,
    source_cover_size: usize,
) {
    if cover.source_quality > cover.carrier_quality {
        println!(
            "Cover Image: {}KB, {}x{}, Progressive YCbCr 4:2:0, Source Q{} capped to Q{} (J-UNIWARD/STC).\n",
            source_cover_size / 1000,
            cover.width,
            cover.height,
        ...
        println!(
            "Cover Image: {}KB, {}x{}, Progressive YCbCr 4:2:0, Source-derived Q{} quantization (J-UNIWARD/STC).\n",
            source_cover_size / 1000,
            cover.width,
            cover.height,
            cover.carrier_quality
...
fn twitter_capacity_limits(
    cover: &twitter::PreparedTwitterCover,
) -> Result<CarrierCapacityLimits, String> {
    carrier_capacity_limits(cover.payload_capacity, twitter::envelope_size)
}

fn print_twitter_capacity_limits(
    cover: &twitter::PreparedTwitterCover,
    limits: CarrierCapacityLimits,
) {
    print_carrier_capacity_limits(
...
fn fail_twitter_payload_capacity(
    cover: &twitter::PreparedTwitterCover,
    source_cover_size: usize,
    stored_payload_size: usize,
) -> Result<(), JdvrifError> {
    let limits = twitter_capacity_limits(cover).map_err(JdvrifError::from)?;
    println!();
    print_twitter_cover_image_summary(cover, source_cover_size);
    println!(
        "Compressed data file (payload) size: {stored_payload_size} bytes ({}KB).\n",
        stored_payload_size / 1000
    );
    print_twitter_capacity_limits(cover, limits);
    let _ = std::io::stdout().lock().flush();
...
fn run_twitter_conceal(
    ...
    let prepared_cover = prepare_twitter_carrier(image_file_path, cover_size)?;

    // A second zlib layer can expand already-compressed formats. The adaptive
    // carrier therefore bypasses compression for recognized types at any size.
    let payload_input = prepare_payload_input(
        data_file_path,
        source_data_size,
    ...
    let filename_prefix_len = 1usize
        .checked_add(data_filename.len())
        .ok_or_else(|| JdvrifError::new("File Size Error: Encrypted X-Twitter output overflow."))?;
    let encrypted_size =
        estimate_prefixed_stream_encrypted_size(stored_payload_size, filename_prefix_len)
            .map_err(JdvrifError::from)?;
    let envelope_size = twitter::envelope_size(encrypted_size).map_err(JdvrifError::from)?;
    if envelope_size > prepared_cover.payload_capacity {
        return fail_twitter_payload_capacity(&prepared_cover, cover_size, stored_payload_size);
    }
    ...
    let encrypted_stage =
        StagingFile::new(Path::new(""), "twitter_enc").map_err(JdvrifError::from)?;
    let stream_header = encrypt_file_with_secretstream_to_file(
        &payload_input.path,
        stored_payload_size,
        &filename_prefix,
    ...
    let mut trailing = [0u8; 1];
    if encrypted_input.read(&mut trailing).map_err(|_| {
        JdvrifError::new("Encryption Error: Failed to read complete X-Twitter encrypted payload.")
    })? != 0
    {
        return Err(JdvrifError::new(
            "Encryption Error: Failed to read complete X-Twitter encrypted payload.",
    ...
    let envelope = Zeroizing::new(
        twitter::make_envelope(&kdf_metadata, &encrypted_data, payload_input.is_compressed)
            .map_err(JdvrifError::from)?,
    );
    let carrier_key = crate::crypto::derive_carrier_key_from_pin(*recovery_pin)
        .map_err(|_| JdvrifError::new("KDF Error: Unable to derive carrier key."))?;
    ...
    .map_err(JdvrifError::from)?;
    let embedded_jpg_size = staged_output
        .staged_size("Write File Error: Failed to verify final X-Twitter output image.")
        .map_err(JdvrifError::from)?;
    ...
    let platforms =
        vec!["X-Twitter. (Only share this \"file-embedded\" JPG image on X-Twitter).".to_string()];
    deliver_conceal_pin(&platforms, *recovery_pin).map_err(JdvrifError::from)?;
    drop(recovery_pin);
    let output_path = staged_output.commit().map_err(JdvrifError::from)?;
    print_conceal_complete(&output_path, embedded_jpg_size).map_err(JdvrifError::from)
...
fn display_reddit_capacity(
    image_file_path: &Path,
    source_cover_size: usize,
) -> Result<(), JdvrifError> {
    let cover = prepare_reddit_carrier(image_file_path, source_cover_size)?;
    let limits = reddit_capacity_limits(&cover).map_err(JdvrifError::from)?;
    let minimum_single_frame_overhead = reddit::envelope_size(
        estimate_prefixed_stream_encrypted_size(0, 2).map_err(JdvrifError::from)?,
    )
    .map_err(JdvrifError::from)?;
    let maximum_single_frame_overhead = reddit::envelope_size(
        estimate_prefixed_stream_encrypted_size(0, 1 + crate::constants::DATA_FILENAME_MAX_LENGTH)
            .map_err(JdvrifError::from)?,
    )
    .map_err(JdvrifError::from)?;
...
consume {minimum_single_frame_overhead} to {maximum_single_frame_overhead} bytes; larger payloads add framing overhead.\n\n\
...
fn display_twitter_capacity(
    image_file_path: &Path,
    source_cover_size: usize,
) -> Result<(), JdvrifError> {
    let cover = prepare_twitter_carrier(image_file_path, source_cover_size)?;
    let limits = twitter_capacity_limits(&cover).map_err(JdvrifError::from)?;
    let minimum_single_frame_overhead = twitter::envelope_size(
        estimate_prefixed_stream_encrypted_size(0, 2).map_err(JdvrifError::from)?,
    )
    .map_err(JdvrifError::from)?;
    let maximum_single_frame_overhead = twitter::envelope_size(
        estimate_prefixed_stream_encrypted_size(0, 1 + crate::constants::DATA_FILENAME_MAX_LENGTH)
            .map_err(JdvrifError::from)?,
    )
    .map_err(JdvrifError::from)?;
...
consume {minimum_single_frame_overhead} to {maximum_single_frame_overhead} bytes; larger payloads add framing overhead.\n\n\
Do not target the theoretical exact limit. When capacity permits, keep the compressed payload at least\n\
1KB below the conservative maximum shown above. The final conceal -x size check is authoritative.\n\
...
pub(crate) fn run_capacity_check(
    ...
    match option {
        ConcealOption::Reddit => {
            ...
            display_reddit_capacity(image_file_path, cover_size)
        }
        ConcealOption::Twitter => {
            let cover_size =
                validate_twitter_cover_for_read(image_file_path).map_err(JdvrifError::from)?;
            display_twitter_capacity(image_file_path, cover_size)
        }
        ConcealOption::None | ConcealOption::Bluesky => Err(JdvrifError::new(
            "Internal Error: Unsupported capsize carrier mode.",
        )),
    }
}
...
pub(crate) fn run_native_conceal(
    ...
    let has_twitter_option = matches!(option, ConcealOption::Twitter);
    ...
    if has_twitter_option {
        let cover_size =
            validate_twitter_cover_for_read(image_file_path).map_err(JdvrifError::from)?;
        validate_twitter_preliminary_limits(cover_size, source_data_size)
            .map_err(JdvrifError::from)?;
        let data_filename = validate_data_filename(data_file_path).map_err(JdvrifError::from)?;
        return run_twitter_conceal(
            image_file_path,
            cover_size,
            data_file_path,
            source_data_size,
