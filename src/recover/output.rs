use crate::common::{has_safe_embedded_filename, StagingFile};
use crate::output_file::commit_staging_file_no_replace;
...
pub(super) fn commit_recovered_output(
    ...
    for attempt in 0..=10000usize {
        ...
        if commit_staging_file_no_replace(staged, &candidate, ERR)? {
            return Ok(candidate);
        }
    }
...
mod tests {
    ...
    fn staged_with(bytes: &[u8]) -> StagingFile {
        use std::io::Write;
        let staged = StagingFile::new(Path::new(""), "test").expect("staging file");
        let mut writer =
            crate::common::open_binary_output_for_staging_or_throw(staged.path()).expect("open");
    ...
    fn commit_recovered_output_refuses_existing_dest() {
        with_temp_cwd(|| {
            let staged = staged_with(b"data");
            fs::write("out.bin", b"old").expect("dest");
            let output = commit_recovered_output(&staged, Path::new("out.bin"))
