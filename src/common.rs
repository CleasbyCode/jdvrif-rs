...
use crate::constants::{
    CORRUPT_FILE_ERROR, MAX_FILE_SIZE, MAX_IMAGE_SIZE, MINIMUM_IMAGE_SIZE,
    REDDIT_UPLOAD_SIZE_LIMIT, TWITTER_UPLOAD_SIZE_LIMIT,
};
...
pub(crate) fn finish_buffered_file(
    ...
    let file = writer.into_inner().map_err(|_| error_message.to_string())?;
...
pub(crate) fn validate_reddit_cover_for_read(path: &Path) -> Result<usize, String> {
    // Reuse the common regular-file, filename, empty-file, and 3 GiB checks,
    // but replace default conceal's 8 MiB ceiling with Reddit's 20 MiB limit.
    let file_size = validate_file_for_read(path, false, false)?;
    if !has_file_extension(path, &["jpg", "jpeg", "jfif"]) {
        return Err("File Type Error: Invalid image extension. Only expecting \".jpg\", \".jpeg\", \".jfif\".".to_string());
    }
    if file_size < MINIMUM_IMAGE_SIZE as usize {
        return Err("File Error: Invalid image file size.".to_string());
    }
...
pub(crate) fn validate_twitter_cover_for_read(path: &Path) -> Result<usize, String> {
    // Reuse common regular-file, filename, empty-file, and 3 GiB checks, then
    // apply X-Twitter's independent 5 MiB input budget.
    let file_size = validate_file_for_read(path, false, false)?;
    if !has_file_extension(path, &["jpg", "jpeg", "jfif"]) {
        return Err("File Type Error: Invalid image extension. Only expecting \".jpg\", \".jpeg\", \".jfif\".".to_string());
    }
...
/// Enforce a caller's allocation limit on the opened file, then read a complete
/// snapshot. A prior path-size check may be stale if the input was replaced.
pub(crate) fn read_file_bounded(path: &Path, max_bytes: usize) -> Result<Vec<u8>, String> {
    crate::signal::check_cancellation()?;
    let mut input = open_binary_input_or_throw(path, "Read Error: Failed to open image file.")?;
    let metadata = input
        .metadata()
        .map_err(|_| "Read Error: Failed to measure opened input file.".to_string())?;
    if !metadata.is_file() || metadata.len() == 0 {
        return Err("Read Error: Input must be a nonempty regular file.".to_string());
    }
    let size = usize::try_from(metadata.len())
        .map_err(|_| "Read Error: Input file exceeds the permitted size limit.".to_string())?;
    if size > max_bytes {
        return Err("Read Error: Input file exceeds the permitted size limit.".to_string());
    }
    let mut bytes = vec![0; size];
    input
        .read_exact(&mut bytes)
        .map_err(|_| "Read Error: Failed to read complete image file.".to_string())?;
    ...
    if input
        ...
        != 0
    {
        return Err("Read Error: Image file changed while being read.".to_string());
    }
    Ok(bytes)
}
...
impl StagingFile {
    ...
    pub(crate) fn new(parent_dir: &Path, tag: &str) -> Result<StagingFile, String> {
        ...
        let (file, can_link) = match open_tmpfile(dir) {
            Some(f) => (f, true),
            None => (
                open_unlinked_named_staging(dir, tag, STAGING_CREATE_ERROR)?,
                false,
            ),
        };
...
mod tests {
    ...
    #[test]
    fn bounded_read_process_probe() {
        let Ok(path) = std::env::var("JDVRIF_READ_PROBE") else {
            return;
        };
        let limit: usize = std::env::var("JDVRIF_READ_LIMIT").unwrap().parse().unwrap();
        match read_file_bounded(Path::new(&path), limit) {
            Ok(bytes) => println!("read {} bytes", bytes.len()),
            Err(error) => {
                assert!(error.contains("permitted size limit"), "{error}");
                println!("bounded read rejected");
            }
        }
    }
