...
/// Used for staging files that may hold ciphertext or plaintext.
...
pub(crate) struct StagingFile {
    // Held solely to keep the descriptor (and therefore the inode) alive; the
    // path below refers to it. Dropping this releases the blocks.
    _file: File,
...
impl StagingFile {
    ...
    pub(crate) fn new(parent_dir: &Path, tag: &str) -> Result<StagingFile, String> {
        ...
        let file = match open_tmpfile(dir) {
            Some(f) => f,
            None => open_unlinked_named_staging(dir, tag, STAGING_CREATE_ERROR)?,
        };
...
mod tests {
    ...
    fn staging_output_is_exclusive_and_mode_0600() {
        ...
        let _f = open_binary_output_for_write_or_throw(&path).expect("create");
        let mode = std::fs::metadata(&path).unwrap().permissions().mode() & 0o777;
        assert_eq!(mode, 0o600);
        assert!(open_binary_output_for_write_or_throw(&path).is_err());
