...
use crate::paths::{unique_randomized_path_or_throw, unique_randomized_path_or_throw_with_token_len};
...
impl OutputFile {
    pub fn create(path: &Path, buffer_capacity: usize) -> Result<OutputFile, String> {
        ...
        Ok(OutputFile { file, buffer: vec![0u8; buffer_capacity.max(1)], fill: 0 })
    }
    ...
    pub fn send_from(&mut self, in_fd: i32, in_offset: usize, length: usize, err: &str) -> Result<(), String> {
    ...
    fn send_from_fallback(&mut self, in_fd: i32, in_offset: usize, length: usize, err: &str) -> Result<(), String> {
...
pub fn commit_staged_file_no_replace(staged: &Path, dest: &Path, err: &str) -> Result<bool, String> {
...
fn copy_file_no_replace(source: &Path, dest: &Path, err: &str) -> Result<bool, String> {
    ...
    let mut output = match OpenOptions::new().write(true).create_new(true).mode(0o600).open(dest) {
        Ok(f) => f,
...
impl<'a> PayloadSource<'a> {
    ...
    pub fn new(template: &'a [u8], encrypted_fd: i32, encrypted_size: usize) -> PayloadSource<'a> {
        PayloadSource { template, template_offset: 0, encrypted_fd, encrypted_offset: 0, encrypted_left: encrypted_size }
    }
    ...
    pub fn copy_to(&mut self, out: &mut OutputFile, size: usize, err: &str) -> Result<(), String> {
        ...
        if self.template_offset < self.template.len() {
            ...
            out.write(&self.template[self.template_offset..self.template_offset + take], err)?;
    ...
    F: FnOnce(&mut OutputFile) -> Result<(), String>,
{
    ...
    let output_path = unique_randomized_path_or_throw_with_token_len(
        parent_dir, out_prefix, out_suffix, MAX_PATH_ATTEMPTS,
        "Write File Error: Could not create a unique output filename.", out_token_hex,
    )?;
    let base = output_path.file_name().map(|v| v.to_string_lossy().into_owned()).unwrap_or_else(|| "output".into());
    let tmp_prefix = format!(".{base}.jdvrif_tmp_");
    let staged = unique_randomized_path_or_throw(
        output_path.parent().unwrap_or(Path::new("")), &tmp_prefix, "", MAX_PATH_ATTEMPTS,
    ...
    let run = (|| -> Result<(), String> {
        ...
        if !commit_staged_file_no_replace(&staged, &output_path, "Write File Error: Failed to commit output image")? {
            return Err("Write File Error: Failed to commit output image: output file already exists".to_string());
        }
        Ok(())
    })();
    if run.is_err() {
...
mod tests {
    ...
    fn hex8() -> String {
        randombytes(4).iter().map(|b| format!("{:02x}", b)).collect()
    }
    ...
    fn payload_source_emits_template_prefix_then_streams_encrypted() {
        ...
        let mut ps = PayloadSource::new(&template, enc_f.as_raw_fd(), enc_f.metadata().unwrap().len() as usize);
