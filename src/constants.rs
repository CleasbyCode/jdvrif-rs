...
You must use the Python script "bsky/bsky_post.py" (in the repo's src folder) to post to Bluesky.
...
// PLATFORM_LIMITS table in conceal_helpers.rs, which omits Bluesky/Reddit and is
// matched by name in finalize_default_platform_report. Keep them in sync with
// the template's ordering.
pub(crate) const BLUESKY_PLATFORM_INDEX: usize = 2;
pub(crate) const REDDIT_PLATFORM_INDEX: usize = 5;
