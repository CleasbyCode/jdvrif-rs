...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/jdvrif-rs-jpeg-safety.XXXXXX")"
trap 'rm -rf "$WORK"' EXIT

FLAGS=(-std=c++20 -O2 -Wall -Wextra -Wpedantic -Wshadow -Wconversion)
...
"${CXX:-g++}" "${FLAGS[@]}" -I"$ROOT" \
    ...
    -ljpeg -lsodium -o "$WORK/test_jpeg_safety"
